...
contract SupplyChain is ConsumerRole, DistributorRole, FarmerRole, RetailerRole, Ownable {
  ...
  function harvestItem(uint  _upc, address  _originFarmerID, string memory _originFarmName, string memory _originFarmInformation, string memory  _originFarmLatitude, string  memory _originFarmLongitude, string  memory g_productNotes, uint _productID) public onlyFarmer()
  {
    // Add the new item as part of Harvest
    Item memory item;
    item.sku = sku;
    // item.ownerID = msg.sender;   Owner is the contract owner
    item.ownerID = owner;
    item.upc = _upc;
    ...
    item.productNotes = g_productNotes;
  ...
  function sellItem(uint   _upc, uint  _price) public verifyCaller(items[_upc].originFarmerID) packed(_upc)
  // Call modifier to check if upc has passed previous supply chain stage
  ...
  function buyItem(uint _upc) public payable forSale(_upc) paidEnough(items[_upc].productPrice) checkValue(_upc) onlyDistributor()
    ...
    Item storage item = items[_upc];
    address farmerId = item.originFarmerID;
   
    item.distributorID = msg.sender;
    item.itemState = State.Sold;
    
    // Transfer money to farmer
    address(uint160(farmerId)).transfer(item.productPrice);
  ...
  function shipItem(uint _upc) public verifyCaller(items[_upc].distributorID) sold(_upc)
    // Call modifier to check if upc has passed previous supply chain stage
